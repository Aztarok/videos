...
//     numberOfThumbnails: number,
//     type: string,
//     cb?: (thumbnail: Blob, index: number) => void
// ): Promise<Blob[]> => {
//     let thumbnail: Blob[] = [];
//     let fractions: number[] = [];
...
//                 let promiseArray = fractions.map(async (time, index) => {
//                     const res = await getVideoThumbnail(videoFile, time); // Pass the current time to getVideoThumbnail
//                     if (cb) cb(res, index);
//                     return Promise.resolve(res);
//                 });
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const promiseArray = Array.from(
                ...
                async (_, index) => {
                    const currentTime = time * index; // Calculate the current time for this thumbnail
                    const res = await getVideoThumbnail(videoFile, time); // Pass the current time to getVideoThumbnail
                    if (cb) cb(res, index);
                    return res;
                }
            ...
            const thumbnails = await Promise.all(promiseArray);
            resolve(thumbnails);
        } catch (error) {
            reject(error);
        }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (typeof videoFile === "string") {
                ...
                importFileandPreview(videoFile).then((urlOfFIle) => {
                    video.src = urlOfFIle;
                });
            }
        } catch (error) {
